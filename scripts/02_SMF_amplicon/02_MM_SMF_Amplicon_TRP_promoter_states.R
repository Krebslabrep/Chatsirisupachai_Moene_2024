...
# Identify promoters that have been detected and sorted by SMF in all samples (DMSO and TRP)
tmp <- do.call(data.frame, lapply(grouped_freq_mats, rowSums))
tmp %>% filter(rowSums(tmp) == 200) %>% row.names() -> keep
...
saveRDS(grouped_freq_mats_filtered, "/g/krebs/chatsiri/mouse_droso_PolII/Chatsirisupachai_Moene_2024/data/MM/amplicon_SMF/MM_TKO_DE_amplicon_TRP_promoter_states_freq_matrix.rds")
...
saveRDS(grouped_freq_mats_collapsed, "/g/krebs/chatsiri/mouse_droso_PolII/Chatsirisupachai_Moene_2024/data/MM/amplicon_SMF/MM_TKO_DE_amplicon_TRP_promoter_states_freq_matrix_collapsed.rds")
