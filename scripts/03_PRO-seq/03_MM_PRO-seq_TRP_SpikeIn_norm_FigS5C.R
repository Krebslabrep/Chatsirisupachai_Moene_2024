########## PRO-seq TKO mESCs: Spike-in normalisation ##########
...
promCounts <- readRDS("/g/krebs/chatsiri/mouse_droso_PolII/Chatsirisupachai_Moene_2024/data/MM/PRO_seq/mouse_TKO_TRP_promoter_counts.rds")
...
promCounts_norm <- promCounts_norm[,c("TKO_0min_R1", "TKO_0min_R2",
                                      "TKO_2.5min_R1", 
                                      "TKO_5min_R1", "TKO_5min_R2",
                                      "TKO_10min_R1", "TKO_10min_R2",
                                      "TKO_20min_R1", "TKO_20min_R2")]

saveRDS(promCounts_norm, "/g/krebs/chatsiri/mouse_droso_PolII/Chatsirisupachai_Moene_2024/data/MM/PRO_seq/mouse_TKO_TRP_promoter_counts_SpikeIn_norm.rds")


##### calculate fold-change for each timepoint and replicate comparing with 0 timepoint
FC_each_rep <- data.frame(TKO_5min_R1 = promCounts_norm$TKO_5min_R1/promCounts_norm$TKO_0min_R1,
                          TKO_5min_R2 = promCounts_norm$TKO_5min_R2/promCounts_norm$TKO_0min_R2,
                          TKO_10min_R1 = promCounts_norm$TKO_10min_R1/promCounts_norm$TKO_0min_R1,
                          TKO_10min_R2 = promCounts_norm$TKO_10min_R2/promCounts_norm$TKO_0min_R2,
                          TKO_20min_R1 = promCounts_norm$TKO_20min_R1/promCounts_norm$TKO_0min_R1,
                          TKO_20min_R2 = promCounts_norm$TKO_20min_R2/promCounts_norm$TKO_0min_R2)
...
time_point_cor <- function(timepoint, df){
  ...
  # plotting
  #pdf(paste0("/g/krebs/chatsiri/mouse_droso_PolII/re_analysis/analysis/qPRO_seq_061123/MM/QC/Fold_change_comparing_with_0min_between_reps", time_point, ".pdf"), 
  #    width = 4.5, height = 4.5, useDingbats=FALSE)
  p <- ggplot(tmp, aes(x = Rep1, y = Rep2)) +
    geom_pointdensity() +
    scale_color_viridis() +
    ggtitle(paste0("Fold change TKO: ", gsub(pattern = "_", replacement = "", timepoint))) +
    xlab("Rep 1") +
    ylab("Rep 2") +
    xlim(c(-8,4)) +
    ylim(c(-8,4)) +
