...
plot_cor <- function(timepoint, df){
  ...
  #pdf(paste0("/g/krebs/chatsiri/mouse_droso_PolII/re_analysis/analysis/qPRO_seq_061123/DM/QC/scatter_between_reps", timepoint, "Promoter.pdf"),
  #    width = 4.5, height = 4.5, useDingbats = FALSE)
  p <- ggplot(tmp, aes(x = .data[[colnames(tmp)[1]]], y = .data[[colnames(tmp)[2]]])) +
    geom_pointdensity() +
    scale_color_viridis() +
    ggtitle(paste0("S2 ", (gsub("_", "", timepoint)))) +
    xlab("log10(RPM) Rep 1") +
    ylab("log10(RPM) Rep 2") +
    annotate("text", x=0.5, y=3.5, label= paste0("r = ", rep_corr), size = 6) +
